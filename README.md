...
| well-known-type-descriptions | `full`, `concise`, or `omit` | How to render comments for [well-known types](https://protobuf.dev/reference/protobuf/google.protobuf/) (`google.protobuf.Timestamp`, `google.protobuf.Duration`, etc.). `full` (default) copies comments from the type's `.proto` file; those comments describe the protobuf representation, which frequently doesn't match JSON. `concise` uses a short JSON-oriented description. `omit` leaves the type undescribed. Descriptions written on your own fields are unaffected. |
