...
| wrap-refs-in-allof         | - | Wrap a `$ref` in an `allOf` when the field also carries sibling keywords (`title`, `description`, protovalidate's `not`, …). Both forms are valid in OpenAPI 3.1, but tooling written against 3.0 drops a `$ref` that has siblings.                              |
