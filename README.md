...
| only-googleapi-http        | - | Only generate routes for methods that have explicit `google.api.http` annotations. Methods without annotations will be skipped.                                   |
...
| plugins                    | `connectrpc;google.api.http;twirp;gnostic;protovalidate` | Semicolon-separated list of plugins to enable. By default, `connectrpc`, `google.api.http`, `gnostic`, and `protovalidate` are enabled. If this option is used, only the specified plugins will be enabled. |
