...
var scenarios = []Scenario{
	...
	{Name: "well_known_type_descriptions_omit", Options: "well-known-type-descriptions=omit"},
	{Name: "well_known_type_descriptions_concise", Options: "well-known-type-descriptions=concise"},
	{Name: "well_known_type_descriptions_full", Options: "well-known-type-descriptions=full"},
	...
	{Name: "with_google_error_detail_googleapi", Options: "features=google.api.http;gnostic;protovalidate,with-google-error-detail"},
...
func TestInt64AsString(t *testing.T) {
	...
	assert.Equal(t, "string", properties["rowCount"].(map[string]any)["type"])
	assert.Equal(t, "int64", properties["rowCount"].(map[string]any)["format"])
